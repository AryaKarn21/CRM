...
export default function Starred() {
   ...
   * { data: [...] }
   *
   * This prevents EmailList from receiving a non-array value.
   */
      ...
      <div className="flex min-h-[300px] items-center justify-center rounded-xl border border-red-200 bg-red-50 p-6 dark:border-red-900/50 dark:bg-red-950/20">
        <div className="text-center">
          <h2 className="text-lg font-semibold text-red-700 dark:text-red-400">
            Failed to load starred emails
          </h2>

          <p className="mt-2 text-sm text-red-600 dark:text-red-400">
            {error?.response?.data?.message ||
              error?.message ||
              "Unable to load starred emails."}
          </p>
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4 lg:h-[calc(100vh-180px)] lg:flex-row lg:gap-6">
      {/* Starred Email List */}
      <section className="w-full min-w-0 lg:w-[38%] lg:min-w-[360px]">
        <EmailList
          emails={emails}
          loading={isLoading}
