...
export default function ContactsList() {
  ...
  const [params, setParams] = useState({ page: 1, limit: 20, search: '', sortKey: 'createdAt', sortDir: 'desc' })
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm({
    defaultValues: { firstName: '', lastName: '', email: '', phone: '', jobTitle: '', department: '', notes: '' }
  })
      ...
      render: (val, row) => (
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>{row.jobTitle || '—'}</p>
          </div>
        </div>
      ),
    },
    { key: 'email', label: 'Email', render: (val) => val ? <a href={`mailto:${val}`} className="text-[var(--primary)] hover:underline" onClick={e => e.stopPropagation()}>{val}</a> : '—' },
  ...
  return (
    <div className="animate-fade-in">
      <div className="page-header">
        <div>
          <h1 className="text-[18px] font-bold" style={{ color: 'var(--text-primary)' }}>Contacts</h1>
          <p className="text-[12px] mt-0.5" style={{ color: 'var(--text-muted)' }}>{data?.total ?? 0} contacts</p>
        </div>
      ...
      <FilterBar searchPlaceholder="Search by name, email..." filters={[]} values={params} onChange={(k, v) => setParams(p => ({ ...p, [k]: v, page: 1 }))} />
        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="form-group">
              ...
              <input className="input" placeholder="e.g. Sales Manager"
                {...register('jobTitle')} />
            </div>
            <div className="form-group">
              <label className="form-label">Department</label>
              <input className="input" placeholder="e.g. Marketing"
                {...register('department')} />
            </div>
          </div>
          <div className="form-group">
