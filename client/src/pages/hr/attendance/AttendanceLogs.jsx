...
export default function AttendanceLogs() {
  ...
  const [params, setParams] = useState({ page: 1, limit: 25, search: '', date: today, status: '' })
  ...
  const { register: regCI, handleSubmit: hCI, reset: resetCI, formState: { errors: errCI } } = useForm({
    defaultValues: { employee: '', date: today, checkIn: '', checkOut: '', status: 'present', notes: '' }
  })
  ...
  const checkInMutation = useMutation({
    ...
    onError: (err) => toast.error(err?.response?.data?.message || 'Failed to log attendance'),
  })
  ...
  const columns = [
    ...
    { key: 'checkOut', label: 'Check Out', render: (val) => val ? format(new Date(val), 'hh:mm a') : <span style={{ color: 'var(--text-muted)' }}>Pending</span> },
    { key: 'hoursWorked', label: 'Hours', render: (val) => val ? <span className="font-medium">{val.toFixed(1)}h</span> : '—' },
    {
      key: 'status', label: 'Status',
      render: (val = 'present') => <Badge variant={statusMap[val] || 'gray'}>{val?.replace('_', ' ')}</Badge>,
    },
        ...
        <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
          {!row.checkOut && (
            <button
              ...
              onClick={() => { if (confirm('Mark check-out now?')) checkOutMutation.mutate(id) }}
              title="Check Out"
            >
              <LogOut size={13} /> Out
            </button>
  ...
  return (
    <div className="animate-fade-in">
      <div className="page-header">
        <div>
          <h1 className="text-[18px] font-bold" style={{ color: 'var(--text-primary)' }}>Attendance Logs</h1>
      ...
      <FilterBar
        ...
        filters={[
          { key: 'status', label: 'Status', options: ['present', 'absent', 'late', 'half_day'].map(v => ({ label: v.replace('_', ' '), value: v })) },
        ]}
        values={params}
      ...
      <div className="mx-6 mb-6 card overflow-hidden">
        <div className="px-4 py-3 border-b flex items-center gap-3" style={{ borderColor: 'var(--border)' }}>
          ...
          <input
            ...
            value={params.date}
            onChange={e => setParams(p => ({ ...p, date: e.target.value, page: 1 }))}
          />
        </div>
        <DataTable
          columns={columns}
      ...
      <FormModal
        ...
        onSubmit={hEdit((d) => updateMutation.mutate({ id: editRecord._id, data: d }))}
